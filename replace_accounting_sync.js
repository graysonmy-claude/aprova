...
const start = 'function AccountingSync({ docs, paymentRecords, setDocs, setPaymentRecords }) {';
const end = 'function UserManagement({ members, setMembers, company }) {';
...
const newBlock = `function AccountingSync({ docs, paymentRecords, setDocs, setPaymentRecords, setViewingDoc, isMobile }) {
                            ...
                            <div style={{ display: isMobile ? "block" : "grid", gridTemplateColumns: isMobile ? "1fr" : "1.1fr 1fr", gap: 16, padding: "20px 24px" }}>
                              ...
                              <div style={{ display: "grid", gap: 14 }}>
                                ...
                                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 8 }}>
                                  ...
                                  <button onClick={() => markInvoicePosted(doc)} disabled={doc.invoicePosted} style={{ padding: "11px 16px", background: "transparent", color: doc.invoicePosted ? "#065F46" : "#1a6fbd", border: \
                                    `1px solid ${doc.invoicePosted ? "#d1fae5" : "#1a6fbd"}`, borderRadius: 10, fontWeight: 700, cursor: doc.invoicePosted ? "default" : "pointer", opacity: doc.invoicePosted ? 0.7 : 1 }}>Post to SQL Account</button>
                                </div>
                                {doc.invoicePosted && (
...
`;
text = text.slice(0, idx) + newBlock + text.slice(idx2);
fs.writeFileSync(filePath, text, 'utf8');
console.log('replaced AccountingSync block');
`;
fs.writeFileSync(path.join(__dirname, 'replace_accounting_sync.js'), newBlock, 'utf8');
